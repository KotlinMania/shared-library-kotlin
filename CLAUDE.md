# Claude Code Project Instructions — shared-library-kotlin

## Project Overview

This is **shared-library-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`shared_library`](https://crates.io/crates/shared_library).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/shared_library/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/shared_library>
